...
public class Main {
	...
	private static final int OK = 1; // Konstanten für den Status der Funktion insertElement() zum Einfügen von
										// Elementen
	private static final int VORHANDEN = 2;
	private static final int GELOESCHT = 1; // Konstanten für den Status der Funktion deleteElement() zum Einfügen von
											// Elementen
	...
	private void initialize() {
		...
		Action addBtnAction = new AbstractAction() { // Aktion als Variable erstellen, um sie sowohl für den addBtn als
														// auch für addTxtF zu nutzen
			@Override
			public void actionPerformed(ActionEvent e) {
				try {
					int insertWert = Integer.parseInt(addTxtF.getText()); // überprüfen, ob eingegebener Text eine ganze
																			// Zahl ist
					if (root == null) { // Inserwert behandelt root == null nicht, also Root setzen, falls nicht
										// vorhanden
						root = new TElement(treePanel);
						root.setWert(insertWert);
						statusMessage.setText("Root mit Wert " + insertWert + " gesetzt");
					} else {
						TElement addElement = new TElement(treePanel); // Element mit dem eingegebenem Wert in den Baum
																		// einfügen
		...
		searchBtn.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				try {
					int searchWert = Integer.parseInt(addTxtF.getText()); // überprüfen, ob eingegebener Wert eine ganze
																			// Zahl ist
		...
		deleteBtn.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				try {
					int deleteWert = Integer.parseInt(addTxtF.getText()); // überprüfen, ob einegegebener Wert eine
																			// ganze Zahl ist
					...
					if (root == null) {
						treePanel.paintComponent(treePanel.getGraphics()); // Panel manuell erneuern, wenn Baum nicht
																			// mnehr gesetzt ist
					}
		...
		@SuppressWarnings("serial")
		Action addRandomBtnAction = new AbstractAction() { // Aktion als Variable erstellen, um sie sowohl für den
															// radnomBtn als auch für randomTxtF zu nutzen
			@Override
			public void actionPerformed(ActionEvent e) {
				try {
	...
	/**
	 * Fügt ein Objekt der Klasse TElement in einem Baum ein param: r: TElement :
	 * aktuelles Element e: TElement : einzufügendes Element return: : int : 0:
	 * Fehler, 1: OK, 2: Element schon im Baum Version 1 Höhe setzen entfernt
	 */
	...
	private static int insertElement(TElement r, TElement e) {
		try {
			...
			if (r.getWert() == e.getWert()) { // überprüfen, ob die Werte des aktuellen Elements und des Einzufügenden
												// Elements gleich sind
				return VORHANDEN;
			} else if (e.getWert() < r.getWert()) { // Links rekursiv einfügen, wenn der Wert des einzufügenden Elements
													// kleiner als des aktuellen Elements ist und
			...
			} else if (e.getWert() > r.getWert()) { // Rechts rekursiv einfügen, wenn der Wert des einzufügenden
													// Elements größer als des aktuellen Elements ist und
	...
	/**
	 * Sucht ein Element nach eingegebenem Wert param: r : TElement : Baum wert: int
	 * : Wert return: : int : 0: Fehler, 1: Gefunden, 2: Nicht gefunden Version 1
	 * Klappt
	 */
	...
	private static TElement FindElement(TElement r, int wert) throws NullPointerException {
		try {
			if (r.getWert() == wert) { // Wenn der Wert des aktuellen Elements dem gesuchten Element gleicht, aktuelles
										// Element zurückgeben
				return r;
			} else {
				if (r.getLeft() != null) {
	...
	/**
	 * Löscht ein Element nach eingegebenem Wert param: r : TElement : Baum wert:
	 * int : Wert Version 3 Komplett überarbeitet
	 */
			...
			if (wert < r.getWert()) {
				...
				if (r.getLeft() != null) {
					if (r.getLeft().getRight() != null) { // LR ersetzen
						...
						r.setLeft(removeBiggestWertElement(r.getLeft())); // Element ohne Element mit höchstem Wert
																			// setzen
					} else { // LL ersetzen
						if (r.getRight() != null) {
							r.getLeft().setRight(r.getRight()); // Rechtes Element an vorgesehenem linkem Element
																// verknüpfen
						}
					...
					if (r.getRight().getLeft() != null) { // RL ersetzen
						...
						r.setRight(removeSmallestWertElement(r.getRight())); // Element ohne Element mit niedrigstem
																				// Wert setzen
					} else { // RR ersetzen
						if (r.getLeft() != null) {
							r.getRight().setLeft(r.getRight()); // Linkes Element an vorgesehenem rechtem Element
																// verknüpfen
						}
	...
	/**
	 * Gibt einen Baum mit PreOrder als String wieder param: r: TElement : Baum
	 * return: : String : PreOrder Text Version 1 Erstellt
	 */
	...
	/**
	 * Gibt einen Baum mit PostOrder als String wieder param: r: TElement : Baum
	 * return: : String : PostOrder Text Version 1 Erstellt
	 */
	...
	/**
	 * Gibt einen Baum mit InOrder als String wieder param: r: TElement : Baum
	 * return: : String : InOrder Text Version 1 Erstellt
	 */
	...
	/**
	 * Zählt die maximale Tiefe param: r: TElement : Baum return: : int : Maximale
	 * Höhe Version 1 Erstellt
	 */
	private static int calMaxHoehe(TElement r, int curHoehe) {
		...
		if (r.getLeft() != null) {
			leftHoehe = calMaxHoehe(r.getLeft(), curHoehe + 1); // Links mit einer Ebene erhöht fortfahren Ergebnis
																// zwischenspeichern
		}
		if (r.getRight() != null) {
			rightHoehe = calMaxHoehe(r.getRight(), curHoehe + 1); // Rechts mit einer Ebene erhöht fortfahren und
	...
	/**
	 * Errechnet die Differenz der Höhe links und rechts param: r: TElement: Baum
	 * curHoehe: int : Aktuelle Höhe return: : int : Differenz der linken und
	 * rechten Höhe Version 1 Erstellt
	 */
	...
	/**
	 * Gibt kleinsten Wert im Baum zurück param: r: TElement : Baum return: : int :
	 * Kleinster Wert von r aus Version 1 Erstellt
	 */
	...
	/**
	 * Gibt höchsten Wert im Baum zurück param: r: TElement : Baum return: : int :
	 * Größter Wert von r aus Version 1 Erstellt
	 */
	...
	/**
	 * Gibt Baum ohne Element mit kleinstem Wert zurück param: r: TElement : Baum
	 * return: : TElement : Baum ohne kleinstes Element von r aus Version 1 Erstellt
	 */
	private static TElement removeSmallestWertElement(TElement r) {
		...
		while (smallestWertElement.getLeft() != null) { // Solange es ein Element mit kleinerem Wert gibt...
			smallestWertParentElement = smallestWertElement; // Aktuelles Element als Vorgängerelement zum Löschen des
																// nächst kleinesten Werts zwischenspeichern
			smallestWertElement = smallestWertElement.getLeft(); // Neues Element mit kleinstem Wert zum überprüfen
																	// zwischenspeichern
		}
	...
	/**
	 * Gibt Baum ohne Element mit höchstem Wert zurück param: r: TElement : Baum
	 * return: : TElement : Baum ohne größtes Element von r aus Version 1 Erstellt
	 */
	private static TElement removeBiggestWertElement(TElement r) {
		...
		while (biggestWertElement.getRight() != null) { // Solange es ein Element mit größerem Wert gibt...
			biggestWertParentElement = biggestWertElement; // Aktuelles Element als Vorgängerelement zum Löschen des
															// nächst Größten Werts zwischenspeichern
			biggestWertElement = biggestWertElement.getRight(); // Neues Element mit größtem Wert zum überprüfen
																// zwischenspeichern
		}
	...
	/**
	 * Unterklasse zur Darstellung des Baumes Version 1 Erstellt
	 */
	@SuppressWarnings("serial")
	public class TreePanel extends JPanel implements treeListener {
		...
		/**
		 * Zeichnet Baum param: g: Graphics: Baum curElement: TElement: aktuelles
		 * Element drawX: int : X Kooirdinate drawY: int : Y Koordinate layer: layer :
		 * Aktuelle Ebene von oben aus gesehen ( 1 = Rootebene ) Version 4 Layer
		 * Parameter hinzugefügt. Annähernde Lösung zur rekursiven Bestimmung der
		 * nächsten x und y Werte gefunden
		 */
		...
		private void drawTree(Graphics g, TElement curElement, int drawX, int drawY, int layer) {
			g.drawOval(drawX - ELEMENT_WIDTH / 2, drawY, ELEMENT_WIDTH, ELEMENT_HEIGHT); // Kreis mittig unter drawX
																							// zeichnen
			String drawStr = String.valueOf(curElement.getWert()); // Zu zeichnender Wert
			g.drawString(drawStr, drawX - g.getFontMetrics().stringWidth(drawStr) / 2,
					drawY + g.getFontMetrics().getHeight() + 4);// Wert zentriert im Kreis zeichnen
			if (curElement.getLeft() != null) { // Links und rechts mit kleiner(-) als - und größer(+) als Wert des
												// aktuellen Elements unterscheiden
				g.drawLine(drawX, drawY + ELEMENT_HEIGHT, drawX - this.getWidth() / (int) (Math.pow(2, layer + 1)),
						drawY + ELEMENT_HEIGHT + ELEMENT_GAP);// Linie von aktuellem zum nächsten Element Zeichnen
				drawTree(g, curElement.getLeft(), drawX - this.getWidth() / (int) (Math.pow(2, layer + 1)),
						drawY + ELEMENT_HEIGHT + ELEMENT_GAP, layer + 1);// Nächstes Element mit X auf die hälfte der
																			// Differenz zum vorherigen Element zeichnen
				// Hälfte der Differenz berechnet aus panelwidth * 1/(2^layer)
			}
			if (curElement.getRight() != null) {
				g.drawLine(drawX, drawY + ELEMENT_HEIGHT, drawX + this.getWidth() / (int) (Math.pow(2, layer + 1)),
						drawY + ELEMENT_HEIGHT + ELEMENT_GAP);// Linie von aktuellem zum nächsten Element Zeichnen
				drawTree(g, curElement.getRight(), drawX + this.getWidth() / (int) (Math.pow(2, layer + 1)),
						drawY + ELEMENT_HEIGHT + ELEMENT_GAP, layer + 1);// Nächstes Element mit X auf das Produkt von
																			// 1,5 und der Differenz zum vorherigen
																			// Element zeichnen
				// Hälfte der Differenz berechnet aus panelwidth * 1/(2^layer)
			}
		}
